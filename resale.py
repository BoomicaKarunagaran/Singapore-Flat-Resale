...
st.set_page_config(page_title='SINGAPORE RESALE FLAT PRICE PREDICTOR',page_icon='house',initial_sidebar_state='expanded',
                        layout='wide',menu_items={"about":'This streamlit application was developed by S.Indumathi'})
...
with _:
    st.empty()
    st.image(r"C:\Users\boomi\OneDrive\Pictures\3e5f0bd0-ed3c-4b28-b515-7b6fd3f7b277.jpg")
...
if selected == "Get Prediction":
    st.write('')
    st.markdown("<h5 style=color:blue>To Predict the Resale Price of a Flat, Please Provide the Following Information:",unsafe_allow_html=True)
    st.write('')
...
if selected == "Home":
    ...
    st.subheader(':rainbow[OVERVIEW OF HDB]')

    st.markdown('''The Housing & Development Board (HDB), also known as the Housing Board, is a statutory body under the 
                Ministry of National Development responsible for public housing in Singapore. 
                Established in 1960 to take over the Singapore Improvement Trust's (SIT) public housing duties, 
                HDB initially focused on building emergency housing and resettling kampong residents into public flats.<br>
                ...
                Singapore's housing market in the 2000s. ''')
    
    with st.container():
        cl,c2,c3=st.columns(3)
        with cl:
                st.subheader(':rainbow[GOAL]')
                st.markdown('''An exceptional organization dedicated to creating homes that everyone can take pride in and cherish.
                            ''')
        with c2:
                st.subheader(':rainbow[TASK]')
                st.markdown('''We deliver affordable, high-quality homes and nurturing living environment where communities can prosper
                            ''')
        with c3:
                st.subheader(':rainbow[STANDARDS]')
                st.markdown('''Our Core Values underpin everything we do. We consistently uphold integrity, foster learning, promote teamwork, strive for excellence, and show care in all our endeavors.
                            ''')
            
    
    st.markdown('<br>', unsafe_allow_html=True)
                
    st.subheader(':rainbow[HERITAGE]')
    #st.markdown('''<h6 style='color:#D0D3D4;font-size:18px'>By the 1940s and 1950s, Singapore experienced rapid population growth, with the 
                ...
                #over the SIT's public housing responsibilities.
                            #''',unsafe_allow_html=True)
    st.markdown('''
                *Population Growth: From 940,700 in 1947 to 1.7 million in 1957.

                *Living Conditions: Worsened, with many in informal settlements or cramped shophouses.

                *SIT Struggles: Low, unsustainable rents; unaffordable for poorer residents.

                *Construction Delays: Approval delays slowed housing construction by 1958.

                *Government Efforts: Mid-1950s, initiated plans for a new housing authority.

                *Housing and Development Bill: Introduced in 1958, passed in 1959.

                *Formation of HDB: Established in February 1960, took over SIT's responsibilities..
                            ''',unsafe_allow_html=True)
...
if selected == "About":
    ...
    st.markdown('<h5>  SINGAPORE RESALE FLAT PRICES PREDICTION ',unsafe_allow_html=True)

    st.subheader(':orange[DOMAIN :]')
    st.markdown('<h5>Real Estate',unsafe_allow_html=True)

    st.subheader(':orange[SKILLS AND TECHNOLOGIES :]')
